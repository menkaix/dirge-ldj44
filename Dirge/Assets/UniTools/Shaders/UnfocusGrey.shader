...
Shader "UIEffects/Unfocus Grey"
{
	Properties
	{
		_BlurAmount("Blur Amount", Range(0,02)) = 0.0005
		_divider("diviseur",int) = 1
	}
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float l = (max(max(texcol.r, texcol.g), texcol.b) + max(max(texcol.r, texcol.g), texcol.b))/(2 * _divider);
				
				texcol.r = texcol.g = texcol.b = l;
